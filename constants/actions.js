export const SET_IS_HOST = 'SET_IS_HOST';
export const SET_ALIAS = 'SET_ALIAS';
export const SET_IS_IN_LOBBY = 'SET_IS_IN_LOBBY';
export const SET_IS_GAME_STARTED = 'SET_IS_GAME_STARTED';
export const SET_ROOM_CODE = 'SET_ROOM_CODE';
export const SET_IS_SINGING = 'SET_IS_SINGING';
export const SET_USER_LIST = 'SET_USER_LIST';
export const SET_LYRICS = 'SET_LYRICS';
export const SET_CURRENT_TURN = 'SET_CURRENT_TURN';
export const SET_IS_ROUND_STARTED = 'SET_IS_ROUND_STARTED';
export const SET_TURN_RESULTS = 'SET_TURN_RESULTS';

export const CREATE_SOCKET = 'CREATE_SOCKET';
export const REMOVE_SOCKET = 'REMOVE_SOCKET';
export const LEAVE_lOBBY = 'LEAVE_LOBBY';
export const UPDATE_ROOM_INFO = 'UPDATE_ROOM_INFO';

export const SET_PLAYLIST = 'SET_PLAYLIST';
export const SET_ACCESS_TOKEN = 'SET_ACCESS_TOKEN';
export const SET_CURRENT_SONG = 'SET_CURRENT_SONG';
export const SET_PLAYING_PARTIAL_SONG = 'SET_PLAYING_PARTIAL_SONG';
export const SET_PLAYING_FULL_SONG = 'SET_PLAYING_FULL_SONG';
